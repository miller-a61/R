...
dim(impact) #rows and columns (dimensions)
nrow(impact) #num rows
ncol(impact) #num columns
...
impact$subject <- factor(impact$subject) #changes class of the variable
class(impact$subject)
...
hist(control[, 3], xlab = "Verbal memory", main = "") #main = "" removes the title 
...
par(mfrow = c(2,3))
plot(density(wmpre[, 4]))
plot(density(pepre[, 4]))
plot(density(dspre[, 4]))
plot(density(wmpost[, 4]))
plot(density(pepost[, 4]))
plot(density(dspost[, 4]))
